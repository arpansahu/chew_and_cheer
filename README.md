...
server {
    ...
    location / {
         proxy_pass              http://{ip_of_home_server}:8001;
         proxy_set_header        Host $host;
         proxy_set_header        X-Forwarded-Proto $scheme;
...
1. Create a new configuration file: Create a new file in the Nginx configuration directory. The location of this directory varies depending on your  operating system and Nginx installation, but it’s usually found at /etc/nginx/sites-available/.
  ...
  ```bash
    touch /etc/nginx/sites-available/chew-and-cheer
    vi /etc/nginx/sites-available/chew-and-cheer
  ```
  ...
  2. Nginx for Kubernetes Deployment 
    ...
    ```bash
        server {
            ...
            location / {
                proxy_pass http://<CLUSTER_IP_ADDRESS>:32001;
                proxy_set_header Host $host;
                proxy_set_header X-Forwarded-Proto $scheme;
...
3.	Enable the new configuration: Create a symbolic link from this file to the sites-enabled directory.
    ...
    ```bash
      sudo ln -s /etc/nginx/sites-available/chew-and-cheer /etc/nginx/sites-enabled/
    ```
    ...
    post {
        success {
            script {
                ...
                // Trigger chew_and_cheer job only if the build is stable
                build job: 'chew_and_cheer', parameters: [booleanParam(name: 'DEPLOY', value: true)], wait: false
            }
        }
        failure {
            script {
...
Make sure to use Pipeline project and name it whatever you want I have named it as per chew_and_cheer
...
Make sure to use Pipeline project and name it whatever you want I have named it as chew_and_cheer_build
...
If image is pushed successfully, email is sent to notify and then another Jenkins Pipeline chew_and_cheer_build is called.
